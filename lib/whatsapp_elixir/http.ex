defmodule WhatsappElixir.HTTP do
  ...
  def get_url(url, opts \\ [], headers \\ []) do
    ...
    case Req.get(url, headers: full_headers) do
      ...
      when status in 200..299 ->
        ...
        if should_json_decode?(content_type) do
          {:ok, Jason.decode!(body)}
        else
          # Return raw binary for media files
          {:ok, body}
        end
  ...
  defp get_content_type(headers) do
    ...
    |> case do
      {_key, content_type} when is_list(content_type) -> List.first(content_type)
      {_key, content_type} -> content_type
      nil -> "application/octet-stream"
    end
  end

  defp should_json_decode?(content_type) do
    content_type
    |> String.downcase()
    |> String.contains?(["application/json", "text/json"])
  end
  ...
  def delete(endpoint, params \\ %{}, opts \\ [], url_override \\ false) do
    ...
    case Req.delete(full_url, headers: headers(token)) do
      ...
      {:ok, %Response{status: status, body: body}} ->
        Logger.error(
          "[WHATSAPP_ELIXIR] HTTP DELETE failed with status #{status}: #{inspect(body)}"
        )
